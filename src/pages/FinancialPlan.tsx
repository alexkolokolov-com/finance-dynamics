    ...
    <main className="bg-background text-foreground">
      ...
      <ArticleSection>
        <ArticleFlow>
          ...
          <p>
            {nbsp("Тогда сначала ищем форму. Это может быть доска Miro, где стикерами формируем ход мыслей. Может быть блокнот, или тетрадь. Да хоть рисунки — главное понять, как вы в принципе планируете жизнь и думаете о ней. А финансовый план уже подстраивается под ваш ход мыслей.")}
          </p>
          <p>
      ...
      <ArticleSection id="stress-test">
        <ArticleFlow>
          ...
          <p>
            {nbsp("Сначала я бы на время забыл про инвестиционные инструменты.")}
          </p>
          <p>
            {nbsp("Допустим, у вас есть те самые 3–5–10 млн рублей. Неважно, где они сейчас лежат — на депозите, брокерском счёте или просто ждут решения.")}
          </p>
          <p>
            {nbsp("Прежде чем куда-то их вкладывать, я бы прогнал эти деньги через 5 простых вопросов.")}
          </p>
          <p>
            {nbsp("❗️ Причём это не тест на финансовую грамотность. Здесь нет правильных и неправильных ответов. Это скорее проверка: понимаете ли вы вообще, какую работу должны выполнить ваши деньги?")}
          </p>

          <H3>{nbsp("1. Когда вам понадобятся эти деньги?")}</H3>
          <p>
            {nbsp("Самый простой вопрос, который почему-то часто пропускают. Если деньги не понадобятся вам 15 лет — это один капитал. Если через 2 года вы хотите купить квартиру — совершенно другой. Если вы пока не знаете, будете ли через год покупать квартиру, переезжать в другую страну или открывать бизнес — третий.")}
          </p>
          <p>
            {nbsp("Хотя сумма на счёте во всех трёх случаях может быть одинаковой.")}
          </p>
          <p>
        ...
        <ArticleFlow className="mt-6 md:mt-8">
          ...
          <p>
            {nbsp("Здесь обычно появляется ответ: «Ну… они должны расти». Но рост капитала сам по себе — довольно бессмысленная цель.")}
          </p>
          <p>
            {nbsp("Вопрос же не в количестве денег на счёте. А в том, что они позволяют сделать.")}
          </p>
          <p>
            {nbsp("Например:")}
          </p>
          <ul className="list-none pl-0">
            ...
            <li>{nbsp("— купить квартиру без ипотеки через 5 лет;")}</li>
            <li>{nbsp("— оплачивать образование детей;")}</li>
            <li>{nbsp("— иметь возможность год не работать;")}</li>
            <li>{nbsp("— переехать;")}</li>
            <li>{nbsp("— открыть свой бизнес;")}</li>
        ...
        <ArticleFlow className="mt-6 md:mt-8">
          ...
          <p>
            {nbsp("Потому что 10 млн рублей могут быть одновременно и большой суммой, и совершенно недостаточным капиталом.")}
          </p>
          <p>
            {nbsp("Как-то ко мне пришёл клиент с вопросом: «Куда мне вложить 18 млн руб?»")}
          </p>
          <p>
            {nbsp("Он рассматривал: недвижимость, облигации, акции, фонды, крипту. Хотел всё диверсифицировать и сделать это максимально грамотно.")}
          </p>
          <p>
            {nbsp("А потом мы начали собирать финансовый план.")}
          </p>
          <p>
            {nbsp("Выяснилось, что")}
          </p>
          <ul className="list-disc pl-5">
            <li>{nbsp("через 2–3 года семья хочет квартиру побольше;")}</li>
            <li>{nbsp("ещё через четыре года старший ребёнок поступает в университет;")}</li>
            <li>{nbsp("плюс человек работает в найме и всерьёз думает в ближайшие годы уйти и запустить собственный бизнес.")}</li>
          </ul>
          <p>
            {nbsp("И внезапно оказывается: никаких 10 млн инвестиционного капитала у него нет.")}
          </p>
          <p>
            {nbsp("Потому что в ближайшие 5 лет они ВСЕ уйдут. Не на одно, так на другое. И только остаток действительно можно будет инвестировать.")}
          </p>
          <p>
            {nbsp("❗️ Причём самое неприятное — первоначальное решение могло выглядеть технически абсолютно правильным. Хороший брокер мог собрать хороший портфель. Хороший консультант по недвижимости подобрать хорошую квартиру.")}
          </p>
          <p>
            {nbsp("Ошибка была бы не в инвестиционном инструменте. Ошибка была бы в том, что человек инвестировал деньги, которые у него есть только номинально. Для инвестиций — их нет!")}
          </p>
          <p>
        ...
        <ArticleFlow className="mt-6 md:mt-8">
          ...
          <p>
            {nbsp("Этот вопрос особенно важен для длинных планов. Потому что на горизонте 10–20 лет почти наверняка что-нибудь пойдёт не по плану 🙂")}
          </p>
          <p>
            {nbsp("Появится ребёнок. Вы решите переехать. Уйдёте из найма. Доход вырастет в два раза. Или временно упадёт. Захочется купить дом. Помочь родителям. Взять год перерыва.")}
          </p>
          <p>
            {nbsp("Поэтому хороший финансовый план — это не таблица, которая пытается угадать вашу жизнь до 2046 года. Он должен выдерживать изменение этой жизни.")}
          </p>
          <p>
        ...
        <ArticleFlow className="mt-6 md:mt-8">
          ...
          <p>
            {nbsp("Наверное, самый странный вопрос для статьи про капитал 🙂. Но иногда лучший способ распорядиться деньгами — не инвестировать их вообще!")}
          </p>
          <p>
            {nbsp("Потратить 1 млн на образование, которое сильно увеличит доход. Вложиться в собственный бизнес. Купить квартиру и убрать огромную статью будущих расходов. Взять полгода без работы. Потратить больше денег на здоровье. Или на путешествия с детьми сейчас, а не когда им будет 25 лет.")}
          </p>
          <p>
            {nbsp("Мы привыкли считать, что финансово грамотный человек должен постоянно превращать деньги в ещё большее количество денег. Но мне кажется, это перевёрнутая логика.")}
          </p>
          <p>
            {nbsp("Деньги существуют для жизни, а не жизнь для увеличения цифры на брокерском счёте.")}
          </p>
          <p>
